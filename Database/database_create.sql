...
CREATE  TABLE IF NOT EXISTS `pksudb`.`files` (
  ...
  `uploaded` DATETIME NULL DEFAULT NULL ,
  PRIMARY KEY (`fileId`) ,
...
CREATE  TABLE IF NOT EXISTS `pksudb`.`eventtypefields` (
  ...
  `fieldDescription` VARCHAR(100) NULL DEFAULT NULL ,
...
CREATE  TABLE IF NOT EXISTS `pksudb`.`filelist` (
  ...
  CONSTRAINT `filelistevent`
    ...
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = 'Table for lists of files.\n\nfieldId: Foreign key referencing  /* comment truncated */ /* /* comment truncated */ /*eventtypefields
